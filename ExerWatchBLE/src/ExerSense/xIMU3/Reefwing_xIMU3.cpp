...
void Reefwing_xIMU3::sendAccessory(char *value)
{
...
void Reefwing_xIMU3::sendResponse(const char *key, const char *value)
{
  char msg[1024];
  sprintf(msg, "Sending %s: %s", key, value);
...
void Reefwing_xIMU3::sendResponse(const char *key, int value) {
  Serial.print("{\"");
  Serial.print(key);
  Serial.print("\":\"");
  Serial.print(value);
...
void Reefwing_xIMU3::checkForCommand() {
  char ximu3_buffer[XIMU3_BUFFER_SIZE];
  
  //  Check for xIMU3 Command Messages
  if (Serial.available() > 0) {
    // read the incoming bytes:
    int blen = Serial.readBytesUntil(TERMINAL, ximu3_buffer, XIMU3_BUFFER_SIZE);
    ...
    for(int i = 0; i < blen; i++) {
      ...
      switch(state) {
        case START_JSON:
          if (ximu3_buffer[i] == STRING_DELIM)
          {
            state = START_CMD;
          }
        break;
        case START_CMD:
          if (ximu3_buffer[i] == STRING_DELIM) { 
            ...
            _cmd[index] = ximu3_buffer[i];
            index++;
          }
        break;
        case END_CMD:
          if (ximu3_buffer[i] == COLON) { state = START_VALUE; }
        break;
        case START_VALUE:
          if (index == 0) {  
            //  Check 1st char of value to detect type
            _valueType = parseValueType(ximu3_buffer[i]);
          }
          if (ximu3_buffer[i] == END_OBJ) { 
            state = END_VALUE; 
            _value[index] = NULL_TERMINATOR;
          }
          else {
            _value[index] = ximu3_buffer[i];
            index++;
          }
        break;
        default:
